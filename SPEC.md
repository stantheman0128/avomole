# 酪梨 AI 家教網（AvoMole）— 黑客松 Demo 規格書

版本 v1.0 ｜ 2026-07-07 ｜ 交付期限：2026-07-07 23:59（台北時間）
狀態：待 Stan 核可後開工
...
| 中文名 | 酪梨 AI 家教網 |
| 英文名 | AvoMole |
| 名稱由來 | AI → 聰明的水果 → 酪梨 avocado → guacamole → **mole**（鼴鼠） |
| 吉祥物 | 一顆戴眼鏡、看起來很聰明的酪梨，騎在一隻鼴鼠上（SVG 手繪風即可） |
| Slogan | 中：「學 AI，找酪梨。」／英：「Smart fruit. Smarter tutors.」 |
...
```
你是「酪梨 AI 家教網（AvoMole）」的媒合助理，一顆聰明友善的酪梨。
...
1. Zeabur 新專案 `avomole`，連 GitHub repo 或直接部署，設定 env `GEMINI_API_KEY`。
2. 先用預設 `avomole.zeabur.app` 類網址上線（今晚交件用這個即可）。
3. 網域 `avomole.dev` 已確認可註冊（約 US$9.99/年）。Stan 若購買，之後在 Zeabur 綁定自訂網域即可，不影響今晚交件。
