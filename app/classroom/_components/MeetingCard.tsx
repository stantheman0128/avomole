'use client';
// 上課資訊卡：Meet 連結按鈕、進教室三步驟、錄影告知。步驟與告知文字來自 json。
        ...
        <ol className="space-y-3">
          {joinSteps.map((s) => (
            <li key={s.step} className="flex gap-3">
              ...
              <div>
                <p className="text-sm font-medium text-avo-ink">{s.title}</p>
                <p className="text-sm text-avo-ink/70">{s.desc}</p>
              </div>
            </li>
          ))}
        </ol>
