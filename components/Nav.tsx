'use client';
// components/Nav.tsx —— 全站頂部導覽。學生主線＝找講師｜AI 媒合；教室體驗不進頂欄。
// Landing（/）用深色版無縫接續；內頁維持近白 bar。登入後保留後臺連結。
...
export function Nav() {
  ...
  const linkCls = onDark ? 'text-avo-paper/80 hover:text-avo-main' : 'text-avo-ink/80 hover:text-avo-main';
    ...
    <header
      className={
        onDark
          ...
          : 'sticky top-0 z-50 border-b border-avo-ink/10 bg-avo-cream/92 backdrop-blur'
      }
    >
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div
            className={`flex items-center overflow-hidden rounded-full border text-xs ${
              onDark ? 'border-avo-paper/30' : 'border-avo-ink/20'
            }`}
          >
            <button
              type="button"
              onClick={() => setLang('zh')}
              className={`px-2.5 py-1 ${lang === 'zh' ? 'bg-avo-main text-avo-dark' : langIdle}`}
            ...
            <button
              type="button"
              onClick={() => setLang('en')}
              className={`px-2.5 py-1 ${lang === 'en' ? 'bg-avo-main text-avo-dark' : langIdle}`}
          ...
          {user ? (
              ...
              <button
                ...
                className={`rounded-full border px-3 py-1.5 text-sm ${
                  onDark
                    ...
                    : 'border-avo-ink/20 text-avo-dark hover:bg-avo-light/50'
                }`}
              >
                {t(NAV.logout)}
              </button>
            ...
            <Link
              ...
              className={
                onDark
                  ? 'rounded-full bg-avo-main px-3 py-1.5 text-sm font-medium text-avo-dark hover:bg-avo-light'
                  : 'rounded-full bg-avo-dark px-3 py-1.5 text-sm text-avo-paper hover:bg-avo-main hover:text-avo-dark'
              }
            >
              {t(NAV.login)}
            </Link>
